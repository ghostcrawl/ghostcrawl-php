...
final class ResponseHelper
{
    ...
    public static function toArray(mixed $value): array
    {
        ...
        // returned an empty [] for /v1/me and /v1/map. Any leftover additionalData
...
class GhostCrawlClient
{
    ...
    {
        $result = ErrorMapper::wait($this->v1->me()->get());
        return ResponseHelper::toArray($result);
    }

    /**
     * Get the current account's identity (alias of {@see me()}).
     * Delegates to GET /v1/me via the generated MeRequestBuilder.
     *
     * @return array<string, mixed>
     */
     ...
     * Delegates to GET /v1/me/usage via the generated UsageRequestBuilder
     * (the me.usage sub-builder). /v1/me/usage is the canonical account-scoped
     * usage endpoint (/v1/budgets/usage requires a budget policy).
     *
     * @return array<string, mixed>
     */
    public function usage(): array
    {
        $result = ErrorMapper::wait($this->v1->me()->usage()->get());
        return ResponseHelper::toArray($result);
    }
     ...
     *
     * @param string      $claimOs      OS to emulate (ios/android/macos/windows/linux).
     * @param string      $claimBrowser Browser to emulate (chrome/firefox/safari).
     * @param string|null $deviceModel  Device model (required for mobile claim_os).
     * @param mixed       $viewport     "fullscreen"/"any" or ['width'=>int,'height'=>int].
     * @param string|null $locale       IETF language tag (e.g. "en-US").
     * @param string|null $timezone     IANA timezone (e.g. "America/New_York").
     * @param bool        $persist      If true the server allocates an identity_id.
     * @param array       $extra        Additional request fields.
     * @return array<string, mixed>
     */
    public function identity(
        ...
        $body = array_merge(
            ['claim_os' => $claimOs, 'claim_browser' => $claimBrowser, 'persist' => $persist],
            $extra
        );
        ...
        return $this->sendJson('POST', '/v1/identity', $body);
    }

    /**
     * List available device models for mobile identity requests.
     * Calls GET /v1/identity/devices through the Kiota request adapter (the
     * generated core has no builder for this route — it is absent from the
     * customer-facing OpenAPI), reusing the canonical auth + base URL + transport.
     *
     * @return array<string, mixed>  {"ios": [...], "android": [...]}
     */
    public function identityDevices(): array
    {
        return $this->sendJson('GET', '/v1/identity/devices', null);
    }
     ...
     * @param string             $path   Absolute API path, e.g. "/v1/identity".
